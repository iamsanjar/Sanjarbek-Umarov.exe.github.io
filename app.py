...
# --------------------------------------------------------------------- css ---
# NOTE: every line below starts at column 0 on purpose. Streamlit's markdown
# renderer treats any line with 4+ leading spaces as a code block, which is
# what caused the raw HTML/CSS to show up as literal text before.
CSS = """
<style>
@import url('https://fonts.googleapis.com/css2?family=JetBrains+Mono:wght@400;500;700;800&display=swap');
html, body, [class*="css"]  { font-family: 'JetBrains Mono', monospace !important; }
.stApp {
background: repeating-linear-gradient(0deg, rgba(61,255,160,0.015) 0px, rgba(61,255,160,0.015) 1px, transparent 1px, transparent 3px), radial-gradient(ellipse at top, #0c130d 0%, #080b08 55%);
color: #c9f2d1;
}
#MainMenu, footer, header { visibility: hidden; }
.termbox{ background:#0d130e; border:1px solid #1c3323; border-radius:8px; padding:18px 22px; margin-bottom:16px; box-shadow:0 20px 60px -20px rgba(0,0,0,.8), 0 0 40px -12px rgba(61,255,160,.06); }
.banner{ color:#3dffa0; font-size:9px; line-height:1.15; white-space:pre; overflow-x:auto; text-shadow:0 0 12px rgba(61,255,160,.25); }
.hl{ color:#ffb454; }
.stat{ color:#ffb454; font-weight:700; }
.sec-title{ color:#3dffa0; font-weight:700; font-size:13px; letter-spacing:.04em; border-bottom:1px solid #1c3323; padding-bottom:6px; margin-bottom:10px; text-transform:uppercase; }
...
.lang-row, .cert-row{ display:flex; justify-content:space-between; font-size:12.5px; padding:5px 0; border-bottom:1px dashed #1c3323; }
...
def md(html: str):
    """Render an HTML fragment, guaranteeing no line has 4+ leading spaces
    (which Streamlit's markdown parser would otherwise render as a code block)."""
...
def boot_sequence():
    ...
    for line in lines:
        shown += line + "&#10;"
        box.markdown(
            "<div class='termbox'><pre class='date' style='float:none;color:#5f8a6d;font-size:12px;white-space:pre-wrap;'>"
            + shown + "</pre></div>",
...
# ---------------------------------------------------------------- render ---
def render_hero():
    # banner must keep its internal newlines/spaces intact (it's ASCII art),
    # so it's built as a single-line f-string rather than passed through md().
    st.markdown(
        f'<div class="termbox"><pre class="banner">{BANNER}</pre>'
        f'<div class="role">DATA_SCIENTIST.exe — Umarov, Sanjarbek</div>'
        f'<div style="color:#5f8a6d;font-size:13px;margin-top:6px;">Economics × Data Science, Westminster International University in Tashkent — turning numbers into decisions.</div>'
        f'<div style="margin-top:12px;">'
        f'<span class="chip">📍 Tashkent, UZ</span>'
        f'<span class="chip">🎓 Class of 2028</span>'
        f'<span class="chip">🐍 Python</span>'
        f'<span class="chip">📊 Power BI</span>'
        f'<span class="chip">🗣 UZ · EN · RU</span>'
        f'</div></div>',
        unsafe_allow_html=True,
    )
...
def render_experience():
    ...
    for job in EXPERIENCE:
        ...
        rows += (
            ...
            f'<span class="role">{job["role"]}</span><span class="date">{job["date"]}</span>'
            f'<div class="org">{job["org"]}</div>'
...
def render_projects():
    ...
    for p in PROJECTS:
        ...
        cards += (
            ...
            f'<div class="proj-desc">{p["desc"]}</div>'
            f'<div>{tags}</div>'
            f'<div style="margin-top:8px;font-size:12px;">→ {p["link"]}</div>'
            f'</div>'
        )
    md(f"""
    <div class="termbox">
      ...
      <div style="color:#5f8a6d;font-size:11.5px;font-style:italic;margin-bottom:10px;">placeholder slots — edit the PROJECTS list in app.py to add your own</div>
      {cards}
    </div>
    """)
...
st.markdown(
    "<div style='text-align:center;color:#3a5943;font-size:11px;margin-top:10px;'>● process exited with code 0 — thanks for scrolling.</div>",
    unsafe_allow_html=True,
)
